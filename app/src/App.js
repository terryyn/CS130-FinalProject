...
function App() {
	...
	const [ currentUserPhotoUrl, setUserPhotoUrl ] = useState(
		'https://cdn2.iconfinder.com/data/icons/animal-outline-icons-set/144/Dog-512.png'
	);
	...
	async function authUser() {
		server.authenticateUser().then((data) => {
			if (data != 'fail') {
				...
				if ('courses' in data)
					if (data.courses && data.courses != '') setCurrentCourses(data.courses.split(','));
			}
		});
	}
					...
					<Route exact path="/">
						{isLoggedIn ? (
							...
							<Login
								...
								setcurrentIsInstructor={setcurrentIsInstructor}
						...
						{isLoggedIn ? (
							<div id="main-page">
								...
								<Home
									...
									currentIsInstructor={currentIsInstructor}
								/>
							</div>
						) : (
							<Redirect to="/" />
